...
SUMM_TS_SIZE
{
	select  substr(ts.TABLESPACE_NAME,1,32) "TS Name",
		...
		substr(ts.PCT_INCREASE,1,5) "PCT Incr",
...
SUMM_TS_USEAGE
{
	select  substr(fs.FILE_ID,1,3) "ID#",
		...
		(100*((sum(fs.bytes))/df.bytes)) "PCT Free",
		df.bytes-sum(fs.bytes) "Bytes Used",
		(100*((df.bytes-sum(fs.bytes))/df.bytes)) "PCT Used"
...
SUMM_TABLE
{
	select  substr(OWNER,1,15) "Owner",
		...
		substr(PCT_FREE,1,3) "PCT Free",
		substr(PCT_USED,1,3) "PCT Used",
	...
	substr(MAX_EXTENTS,1,5) "MaxEx",
		substr(PCT_INCREASE,1,5) "PCT Incr"
	from sys.dba_tables
...
OV_LIB_CACHE_DETAIL
{
	select namespace "Namespace", 
		pins "Pins", 
		pinhits "Pin Hits", 
		pinhitratio*100 "Pinhit Ratio", 
		reloads "Reloads", 
		reloads/decode(pins, 0, 1, pins)*100 "Reload Ratio"
	from v$librarycache
	order by "Pinhit Ratio"
}
...
OV_SHARED_POOL_SIZE
{
	select to_number(v$parameter.value) "Shared Pool Size", 
		...
		(v$sgastat.bytes/to_number(v$parameter.value))*100 "PCT Free"
...
OV_SHARED_POOL_RESERVED
{
	select request_failures "Request Failures", 
		last_failure_size "Last Failure Size", 
		avg_free_size "Avg Free Size"
	from v$shared_pool_reserved;
}

OV_SHARED_POOL_RESERVED_SMALL
{
	select request_failures "Rqst Failure", 
		last_failure_size "Failure Size", 
		max_free_size "Max Free Size", 
		free_space "Free Space"
	from v$shared_pool_reserved
}

OV_SHARED_POOL_RESERVED_LARGE
{
	select request_misses "Request Misses"
	from v$shared_pool_reserved
}

OV_SYS_SHM_DETAIL
{
	select  ksmchcls "Status", 
		sum(ksmchsiz) "Bytes"
	from x$ksmsp
	group by ksmchcls;
}

OV_SESSION_CACHE_CURSORS
{
	select name "Name", 
		value "Value"
	from v$sysstat 
...
MEM_REDO_RATIO
{
	select decode(s1.value, 0, 1, s1.value)/decode(s2.value, 0, 1, s2.value)*100 "PCT Request/Entry"
...
MEM_REDO_WAIT_TIME
{
	select name "Name", 
		value "Value"
	from v$sysstat
	where name in ('redo log space wait time')
}
...
DBA_EXTENTS
{
	select e.tablespace_name "TS Name",
		...
		sum(e.bytes)/df.bytes*100 "PCT Used",
		autoextensible "AutoExt"
...
DBA_OBJ_SIZE_SUMM
{
	select type "Type",
		count(name) "Counts",
	      ...
	      (sum(source_size) +sum(parsed_size) +sum(code_size) +sum(error_size))/1024 "SizeRequired (KB)"
...
USER_LOCK_DETAIL
{
   SELECT 
   	m.sid "Sid", 
   	sn.username "Username", 
         obj1.object_name "Obj1 Name", 
         obj2.object_name "Obj2 Name",
   	m.type "Type",
   DECODE(m.lmode, 0, 'None'
                 , 1, 'Null'
                 , 2, 'Row Share'
...
STAT_SQL_EXEC_COUNT
{
	SELECT PARSE_CALLS "Parse Calls", 
		EXECUTIONS "Exec Counts", 
		SQL_TEXT "SQL"
	FROM V$SQLAREA
...
STAT_SQL_PARSE_EXEC
{
	select s.sid "Sid", 
		...
		sn.name "Stat Name", 
		s.value "Value"
	from v$sesstat s, v$session se, v$statname sn
	where s.sid = se.sid
	and s.statistic# = sn.statistic#
	and sn.name in ('parse count (total)', 'parse count (hard)', 'execute count')
	and username is not null
	order by s.sid
}
...
STAT_SQL_BUFFER
{
	SELECT BUFFER_GETS "Buffer Gets", 
...
STAT_WAIT_EVENT_BACKGROUND
{
	select event "Event", 
		sum(total_waits) "Waits",
		sum(time_waited) "Time"
	from v$session s, v$session_event e
	where s.sid  = e.sid
	and type = 'BACKGROUND'
	group by event
...
{
	select v.sid "Sid", 
		se.username "User Name", 
		v.value "Value"
	from v$statname s, v$sesstat v, v$session se
...
SPACE_TS_FREE_SPACE
{
	select a.tablespace_name "TS Name",
		...
		sum(a.sumb)*100/sum(a.tots) "PCT Free",   
...
LOCK_LATCH_HIT_RATIO
{
	select name "Name",
		sleeps "Sleeps",
		((gets - misses) * 100) / decode(gets, 0, 1, gets) "PCT Get",
		((immediate_gets - immediate_misses) * 100) 
			/ decode(immediate_gets, 0, 1, immediate_gets) "PCT Imm Get"
	from v$latch
	where gets <> 0 or immediate_gets <> 0
	order by name;
}
...
MON_ROLLSTAT
{
	select a.name, gets, waits, writes, rssize, xacts, shrinks, wraps, optsize
	from v$rollname a, v$rollstat b
	where a.usn = b.usn
}

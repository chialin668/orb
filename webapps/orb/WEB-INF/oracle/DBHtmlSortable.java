...
public class DBHtmlSortable extends Database {
	...
	*
	**/
	public boolean executeSQL(String sqlTag, String callerURL,
								String chkTag, String sortColName, JspWriter out) {
		String METHODNAME = "executeSQL";
		return this.executeSQL(sqlTag, callerURL, chkTag, sortColName, "y", null);
	...
	**/
	public boolean executeSQL(String sqlTag, String callerURL,
								String chkTag, String sortColName, String desc, JspWriter out) {
		...
		if (chkTag != null && sqlTag.equals(chkTag)) {
			...
			if (sortColName != null) {
				...
				} else if (desc.equals("y")) {
					newSql = newSql + "order by \"" + sortColName + "\" desc";
					desc = "n";

				} else if (desc.equals("n")) {
					newSql = newSql + "order by \"" + sortColName + "\"";
					desc = "y";
				}
				sqlStr = newSql;
			}
...
*/
				...
				for (int i=1;i<=colCount;i++) {
					...
					// null will screw up the table layout?
					if (nextData == null)
						nextData = "-";

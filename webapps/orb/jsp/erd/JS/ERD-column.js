...
*
**/
function co_GetPrimaryKeyTd(columnTr)	{ return columnTr.firstChild; }
function co_GetColumnNameTd(columnTr) 	{ return co_GetPrimaryKeyTd(columnTr).nextSibling; }
function co_GetDataTypeTd(columnTr) 	{ return co_GetColumnNameTd(columnTr).nextSibling; }
function co_GetDataLengthTd(columnTr) 	{ return co_GetDataTypeTd(columnTr).nextSibling; }
function co_GetNotNullTd(columnTr) 		{ return co_GetDataLengthTd(columnTr).nextSibling; }
function co_GetValidateTd(columnTr) 	{ return co_GetNotNullTd(columnTr).nextSibling; }
...
function co_GetPrimaryKey(columnTr) 	{ return co_GetPrimaryKeyTd(columnTr).firstChild.data; }
function co_GetColumnName(columnTr) 	{ return co_GetColumnNameTd(columnTr).firstChild.data; }
function co_GetDataType(columnTr) 		{ return co_GetDataTypeTd(columnTr).firstChild.data; }
function co_GetDataLength(columnTr) 	{ return co_GetDataLengthTd(columnTr).firstChild.data; }
function co_GetNotNull(columnTr) 		{ return co_GetNotNullTd(columnTr).firstChild.data; }
function co_GetValidate(columnTr) 		{ return co_GetValidateTd(columnTr).firstChild.data; }
function co_GetDefault(columnTr) 		{ return co_GetDefaultTd(columnTr).firstChild.data; }
**/
//////////////////////////////////////////////////////////////////////////////////////////////////////////
...
*
**/
function co_GetNotNullByColTr(columnTr) {
	var pKeyTd = columnTr.firstChild;
	var columnTd = pKeyTd.nextSibling;
	var typeTd = columnTd.nextSibling;
	var lengthTd = typeTd.nextSibling;
	...
	var object = notNullTd.firstChild;
...
*
**/
function co_IsForeighKey(relationType, columnTr) {

	if (relationType == null) 
		relationType = MOD_ONE2MANY;

	var pKeyTd = co_GetPrimaryKeyTd(columnTr);
	var object = pKeyTd.firstChild;
	...
	//
	// this key part of the parent primary key? (for one to one relation) MOD_ONE2ONE
	//	
	// MOD_ONE2ONE and MOD_SELECT only --> don't want to modify one 2 one child p key!!
	//
	if (relationType == MOD_ONE2ONE || relationType == MOD_SELECT) {	 
		...
		for (var i=0;i<parentArray.length;i++) {
			...
			var tbody = tb_GetHtmlTableBody(pTableName);

			pColumnTr = tbody.firstChild;

			while (pColumnTr != null) {

				if (co_IsPrimaryKey(pColumnTr)) {

					 var pColumnName = co_GetColNameByColTr(pColumnTr);

					 if (columnName == pColumnName)
						return true;
				}

				pColumnTr = pColumnTr.nextSibling;
			}
		}
	}	
...
*
**/
function co_ColumnTrInput(primaryKey, columnName, dataType, dataLength, notNull, validate, dftValue) {

	if (primaryKey==null || columnName==null || dataType==null || dataLength==null 
			|| notNull==null || validate==null || dftValue==null) {
	...
	columnTd.className = CL_COLUMN_NAME;		
		...
		inputTxt.ondeactivate = co_ValidateColumnName;
		inputTxt.type = "text";
	...
	lengthTd = document.createElement("TD");
		...
		var inputTxt = document.createElement("input");
		inputTxt.ondeactivate = co_ValidateDataLength;
		inputTxt.type = "text";
	...
	// notNull
	var notNullTd = document.createElement("TD");
	notNullTd.className = CL_NULLABLE;
		var checkBox = document.createElement("input");
		checkBox.type = "checkbox";
		checkBox.value = notNull;
	notNullTd.appendChild(checkBox);
	tr.appendChild(notNullTd);
...
function co_ColumnTr(primaryKey, columnName, dataType, dataLength,
	...
	var notNullTd = document.createElement("TD");
	notNullTd.className = CL_NULLABLE;
	var text = document.createTextNode(notNull);
	notNullTd.appendChild(text);
	tr.appendChild(notNullTd);
...
function co_PrimaryKeyControl() {  

	var pKeyChkBox = this;
	var pKeyTd = pKeyChkBox.parentElement;
	var columnTr = pKeyTd.parentElement;

	var notNullTd = co_GetNotNullTd(columnTr);
	var notNullChkBox = notNullTd.firstChild;

	if (pKeyChkBox.checked) {

		//
		// make sure we don't have the same column in
		...
		for (var i=0;i<childArray.length;i++) {
			...
			if (tb_HasColumn(childTableName, columnName)) {
			
				pKeyChkBox.checked = false;
				pKeyChkBox.focus();

				alert("Found same column name in child table!!\n" 
						+ "   column: " + childTableName + "." + columnName);
		...
		//
		notNullChkBox.disabled = true;
		notNullChkBox.checked = true;
		
	} else {
	
		notNullChkBox.disabled = false;
		notNullChkBox.checked = false;
...
function co_ColumnTextToInput(columnTr) {
	...
	var pKeyChkBox = ut_TextToCheckbox(text);
		pKeyChkBox.onclick = co_PrimaryKeyControl;
	pKeyTd.replaceChild(pKeyChkBox, text);

	// column name
	var columnTd = pKeyTd.nextSibling;
	var text = columnTd.firstChild;
	var columnName = ut_TextToInput(text, null, null);
	columnName.ondeactivate = co_ValidateColumnName;
	columnTd.replaceChild(columnName, text);
	...
	var dataLength = ut_TextToInput(text, 3, "right");
	dataLength.ondeactivate = co_ValidateDataLength;
	lengthTd.replaceChild(dataLength, text);

	// not null
	var notNullTd = lengthTd.nextSibling;
	var text = notNullTd.firstChild;
	var notNullChkBox = ut_TextToCheckbox(text);
	if (pKeyChkBox.checked) notNullChkBox.disabled = true;
	notNullTd.replaceChild(notNullChkBox, text);
...
*
**/
function co_IsInput(columnTr) {

	var pKeyTd = columnTr.firstChild;
	var checkbox = pKeyTd.firstChild;

	if (checkbox.tagName == null)  // not an input
		return false;
...
function co_ColumnInputToText(columnTr) {
	...
	var notNullTd = lengthTd.nextSibling;
	var checkbox = notNullTd.firstChild;
	notNullTd.replaceChild(ut_CheckBoxToText(checkbox), checkbox);
...
*
* @relationType		MOD_ONE2ONE or MOD_ONE2MANY
* @tableName		Name of the child table that we want to add foreign keys to
* @pKeyColumnArray	Primay Key Array
*
* @return	boolean
*
**/
function co_AddFKeyTrArray(relationType, tableName, pKeyColumnArray) {

	if (tableName == null || pKeyColumnArray == null) {
		var errMsg = "One of the following string is null:"
				+ " tableName: " + tableName
				+ ", columnArray: " + columnArray 
				+ "\n";
	...
	if (pKeyColumnArray.length == 0) 
		return true;


	// define it's a primary key or foreign key
	var keyType = FOREIGN_EKY_MARK;
	
	if (relationType == MOD_ONE2ONE) 
		keyType = PRIMARY_EKY_MARK;
		
	else if (relationType == MOD_ONE2MANY) 
		keyType = FOREIGN_EKY_MARK;


	for (var i=0; i<pKeyColumnArray.length; i++) {
		var pKeyColTr = pKeyColumnArray[i];

		//
		// if we have this foreign key alreay??
		//
		var columnTr = tb_GetColumnTr(tableName, co_GetColNameByColTr(pKeyColTr));
		
		
		if (columnTr != null && co_IsForeighKey(relationType, columnTr)) {
			// has one already 

			continue;
			
		} else if (columnTr != null && !co_IsForeighKey(relationType, columnTr)) {

			// change this column to foreign key
			var pKeyTd = co_GetPrimaryKeyTd(columnTr);
			var pKey = pKeyTd.firstChild;
			pKey.data = FOREIGN_EKY_MARK;
			
		} else {

			//
			// add this new primary(MOD_ONE2ONE)/foreign(MOD_ONE2MANY) key
			//
			var fKeyColTr = co_ColumnTr(keyType, 				// key type
					co_GetColNameByColTr(pKeyColTr),			// column name
					co_GetTypeByColTr(pKeyColTr), 				// data type
					co_GetLengthByColTr(pKeyColTr),				// data length
					co_GetNotNullByColTr(pKeyColTr),			// notNull
	...
	//
	// reorganize the columns (move the NEW primary key(s) up)
	//
	if (relationType == MOD_ONE2ONE) {
...
function co_EditColumn(columnTd) {
	...
	if (co_IsForeighKey(null, columnTr)) {
		...
		if (childArray.length > 0) {
			...
			for (var i=0;i<childArray.length;i++) {
				...
				var fKeyColTr = co_ColumnTr(FOREIGN_EKY_MARK, 	// foreign key!!
						...
						co_GetNotNullByColTr(columnTr),// notNull
		...
		if (childArray.length > 0) {
			...
			for (var i=0;i<childArray.length;i++) {
				...
				while(cTr != null) {
					...
					if (co_IsForeighKey(null, cTr) && columnName == childColName) {
...
function co_EditApply() {
	...
	//
	// dont' allow to edit table name (we're in the 'column' edit mode!!)
	//
	var tableDiv = tb_GetTableDivByButton(this);
	var tableName = tableDiv.id;
	...
	//
	// chnage input to text for this column
	//
	var columnTr = tb_GetChosenColTr(tableName);
	...
	//
	//	the buttons --> change from column controlTr to table controlTr
	//
	var controlTr = tb_GetControlTr(tableName);
	if (controlTr == null) {
		sy_logError("ERD-table", 
...
*
**/
function co_ValidateColumnName() {

	var columnName = this.value;
	
	if (!PATTERN_VALID_NAME.test(columnName)) {
...
*
**/
function co_ValidateDataLength() {

	var dataLength = this.value;
	
	if (dataLength == "") 
		this.value = "-"
	
	if (!PATTERN_DATA_LENGTH.test(dataLength) 
			&& !PATTERN_EMPTY_STRING.test(dataLength)) {
		
		alert("Data length should be numbers only.");

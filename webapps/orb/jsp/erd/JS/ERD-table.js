...
*
**/
function tb_GetHtmlLongTableBody(tableName) {
...
function refreshShortView(tableName) {
	...
	// change it to long view
	var newShortTableBody = tb_CreateShortTableBody(longTableBody);
//	htmlTable.replaceChild(newShortTableBody, shortTableBody);

}

//###
function tb_CreateShortTableBody(longTableBody) {

	var shortTableBody = longTableBody.cloneNode(true);
/*	
	var columnTr = shortTableBody.firstChild;
	
	while(columnTr != null) {

		var primaryKey = co_GetPKeyByColTr(columnTr);

		var columnNameTd = co_GetColumnNameTd(columnTr);
		var columnName = co_GetColNameByColTr(columnTr);
		var columnNameTxt = columnNameTd.firstChild;
		
		var dataTypeTd = co_GetDataTypeTd(columnTr);
		var dataType = co_GetTypeByColTr(columnTr);
		
		var dataLengthTd = co_GetDataLengthTd(columnTr);
		var dataLength = co_GetLengthByColTr(columnTr);
	
		columnNameTxt.data = columnName + ": "
							+ dataType + "(" + dataLength + ")";
		
		var notNullTd = co_GetNotNullTd(columnTr);
		var validateTd = co_GetValidateTd(columnTr);
		var defaultTd = co_GetDefaultTd(columnTr)
		
		columnTr.removeChild(dataTypeTd);
		columnTr.removeChild(dataLengthTd);
		columnTr.removeChild(notNullTd);
		columnTr.removeChild(validateTd);
		columnTr.removeChild(defaultTd);
		
		columnTr = columnTr.nextSibling;
	}
*/	
	return shortTableBody;
}
...
function tb_AddForeignkey(childTableName, parentColumnTr) {
	...
	//
	// we don't have the key, so create a new foreign key
	//
...
* 
**/
function tb_RemoveForeignKey(childTableName, parentColumnTr) {

	var parentColumnName = co_GetColNameByColTr(parentColumnTr)
	...
	while(columnTr != null) {
		...
		var childColumnName = co_GetColNameByColTr(columnTr);

		if ((childColumnName == parentColumnName && co_IsForeighKey(MOD_ONE2MANY, columnTr)) 
						||
				(childColumnName == parentColumnName && co_IsPrimaryKey(columnTr)) )  { // found!!
...
function tb_ForeignKeyControl(tableName) {
	...
	while(columnTr != null) {
		...
		if (co_IsPrimaryKey(columnTr)) {
			...
			// remove
			for (var i=0;i<childArray.length;i++) {
				tb_RemoveForeignKey(childArray[i], columnTr);
				refreshShortView(childArray[i]);
			}
		}

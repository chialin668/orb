...
/*
		...
		var tableName = NEW_TABLE;
		var columnName = EMPTY_STRING;
		chosenColumn = columnName;

		var columnTr = tb_GetColumnTr(tableName, columnName);
		if (columnTr == null) {
			sy_logError("ERD-column", 
					"ev_MouseDown", 
		...
		var columnTd = co_GetColumnTd(columnTr);
		if (co_EditColumn(columnTd) == false) {
			sy_logError("ERD-column", 
					"ev_MouseDown", 
					"Error editing the new column!!");
			return;
		}
*/
	...
	/////////////////////////////////////
		...
		if (event.srcElement.className == CL_DB_TABLE) {
			...
			if (tb_HasPrimaryKey(chosenTableName) == false) 
